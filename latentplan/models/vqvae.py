...
class VQEmbeddingMovingAverage(nn.Module):
    def __init__(self, K, D, decay=0.99):
        ...
        embedding.uniform_(-1.0 / K, 1.0 / K)
        self.decay = decay
...
class VQStepWiseTransformer(nn.Module):
    def __init__(self, config, feature_dim):
        ...
        self.pos_emb = nn.Parameter(
            torch.zeros(1, self.trajectory_length, config.n_embd)
        )
        ...
        self.latent_mixing = nn.Linear(
            self.latent_size + self.observation_dim, self.embedding_dim
        )
        ...
        if self.bottleneck == "pooling":
            self.latent_pooling = nn.MaxPool1d(
                self.latent_step, stride=self.latent_step
            )
        elif self.bottleneck == "attention":
            self.latent_pooling = AsymBlock(
                config, self.trajectory_length // self.latent_step
            )
    ...
    def decode(self, latents, state):
        ...
        joined_pred[:, :, : self.observation_dim] += torch.reshape(
            state, shape=[B, 1, -1]
        )
        return joined_pred
    ...
    def forward(self, joined_inputs, state):
        ...
        if self.bottleneck == "attention":
            if self.masking == "uniform":
                mask = torch.ones(latents_st.shape[0], latents_st.shape[1], 1).to(
                    latents_st.device
                )
                mask_index = np.random.randint(
                    0, latents_st.shape[1], size=[latents_st.shape[0]]
                )
...
class VQContinuousVAE(nn.Module):
    ...
    def configure_optimizers(self, train_config):
        ...
        if isinstance(self.model, SymbolWiseTransformer) or isinstance(
            self.model, VQStepWiseTransformer
        ):
    ...
    def encode(self, joined_inputs, terminals):
        b, t, joined_dimension = joined_inputs.size()
        padded = torch.tensor(
            self.padding_vector, dtype=torch.float32, device=joined_inputs.device
        ).repeat(b, t, 1)
        ...
        trajectory_feature = self.model.encode(
            torch.cat([joined_inputs, terminals], dim=2)
        )
    ...
    def decode_from_indices(self, indices, state):
        B, T = indices.shape
        if self.model.ma_update:
            latent = torch.index_select(
                self.model.codebook.embedding, dim=0, index=indices.flatten()
            ).reshape([B, T, -1])
        else:
            latent = torch.index_select(
                self.model.codebook.embedding.weight, dim=0, index=indices.flatten()
            ).reshape([B, T, -1])
        if self.model.bottleneck == "attention":
            latent = torch.concat(
                [
                    ...
                    torch.zeros(
                        [
                            B,
                            self.trajectory_length // self.latent_step,
                            latent.shape[2],
                        ],
    ...
    def forward(
        self, joined_inputs, targets=None, mask=None, terminals=None, returnx=False
    ):
        """
        ...
        if terminals is not None:
            terminal_mask = torch.clone(1 - terminals).repeat(
                1, 1, joined_inputs.shape[-1]
            )
            joined_inputs = joined_inputs * terminal_mask + (1 - terminal_mask) * padded
        ...
        ## [ B x T x embedding_dim ]
        # forward the GPT model
        reconstructed, latents, feature = self.model(
            torch.cat([joined_inputs, terminals], dim=2), state
        )
        pred_trajectory = torch.reshape(
            reconstructed[:, :, :-1], shape=[b, t, joined_dimension]
        )
        ...
        if targets is not None:
            ...
            weights = torch.cat(
                [
                    ...
                    torch.ones(self.action_dim, device=joined_inputs.device)
                    * self.action_weight,
            ...
            mse = (
                F.mse_loss(pred_trajectory, joined_inputs, reduction="none")
                * weights[None, None, :]
            )

            first_action_loss = self.first_action_weight * F.mse_loss(
                joined_inputs[
                    :, 0, self.observation_dim : self.observation_dim + self.action_dim
                ],
                pred_trajectory[
                    :, 0, self.observation_dim : self.observation_dim + self.action_dim
                ],
            )
            sum_reward_loss = self.sum_reward_weight * F.mse_loss(
                joined_inputs[:, :, -2].mean(dim=1),
                pred_trajectory[:, :, -2].mean(dim=1),
            )
            last_value_loss = self.last_value_weight * F.mse_loss(
                joined_inputs[:, -1, -1], pred_trajectory[:, -1, -1]
            )
            cross_entropy = F.binary_cross_entropy(
                pred_terminals, torch.clip(terminals.float(), 0.0, 1.0)
            )
    ...
    def forward(self, idx, state, targets=None):
        ...
        if not idx is None:
            ...
            token_embeddings = torch.cat(
                [
                    torch.zeros(size=(b, 1, self.embedding_dim)).to(token_embeddings),
                    token_embeddings,
                ],
                dim=1,
            )
