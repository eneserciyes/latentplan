...
class SequenceDataset(torch.utils.data.Dataset):
    def __init__(
        ...
        dataset = qlearning_dataset_with_timeouts(
            env.unwrapped, terminate_on_end=True, disable_goal=disable_goal, debug=False
        )
        ...
        self.obs_mean, self.obs_std = observations.mean(
            axis=0, keepdims=True
        ), observations.std(axis=0, keepdims=True)
        self.act_mean, self.act_std = actions.mean(axis=0, keepdims=True), actions.std(
            axis=0, keepdims=True
        )
        ...
        self.joined_raw = np.concatenate(
            [observations, actions], axis=-1, dtype=np.float32
        )
        ...
        if normalize_raw and normalize_reward:
            ...
            self.values_segmented = (
                self.values_segmented - self.value_mean
            ) / self.value_std
            self.rewards_segmented = (
                self.rewards_segmented - self.reward_mean
            ) / self.reward_std
        else:
        ...
        self.joined_segmented = np.concatenate(
            [
                ...
                np.zeros(
                    (n_trajectories, sequence_length - 1, joined_dim), dtype=np.float32
                ),
            ],
            axis=1,
        )
    ...
    def normalize_states(self, states):
        ...
        states = (states - obs_mean) / (obs_std + 1e-8)
        return states
    ...
    def __getitem__(self, idx):
        ...
        terminal = 1 - torch.cumprod(
            ~torch.tensor(
                self.termination_flags[path_ind, start_ind : end_ind : self.step, None]
            ),
            dim=1,
        )
    ...
    def get_test(self):
        ...
        for path_ind, start_ind, end_ind in self.test_indices:
            ...
            terminal = 1 - torch.cumprod(
                ~torch.tensor(
                    self.termination_flags[
                        path_ind, start_ind : end_ind : self.step, None
                    ]
                ),
                dim=1,
            )
